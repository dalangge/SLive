...
static ngx_int_t ngx_http_live_stat_postconfiguration(ngx_conf_t *cf);
static void * ngx_http_live_stat_create_loc_conf(ngx_conf_t *cf);
static char * ngx_http_live_stat_merge_loc_conf(ngx_conf_t *cf,
...
#define NGX_HTTP_LIVE_STAT_ALL           0xff
#define NGX_HTTP_LIVE_STAT_GLOBAL        0x01
#define NGX_HTTP_LIVE_STAT_LIVE          0x02
#define NGX_HTTP_LIVE_STAT_CLIENTS       0x04
#define NGX_HTTP_LIVE_STAT_PLAY          0x08
...
} ngx_http_live_stat_loc_conf_t;


static ngx_conf_bitmask_t           ngx_http_live_stat_masks[] = {
    { ngx_string("all"),            NGX_HTTP_LIVE_STAT_ALL           },
    { ngx_string("global"),         NGX_HTTP_LIVE_STAT_GLOBAL        },
    { ngx_string("live"),           NGX_HTTP_LIVE_STAT_LIVE          },
    { ngx_string("clients"),        NGX_HTTP_LIVE_STAT_CLIENTS       },
    { ngx_null_string,              0 }
};


static ngx_command_t  ngx_http_live_stat_commands[] = {
    
    { ngx_string("http_live_stat"),
        NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_1MORE,
        ngx_http_live_stat,
        NGX_HTTP_LOC_CONF_OFFSET,
        offsetof(ngx_http_live_stat_loc_conf_t, stat),
        ngx_http_live_stat_masks },
    
    { ngx_string("http_live_stat_stylesheet"),
        NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_CONF_TAKE1,
        ngx_conf_set_str_slot,
        NGX_HTTP_LOC_CONF_OFFSET,
        offsetof(ngx_http_live_stat_loc_conf_t, stylesheet),
        NULL },
...
static ngx_http_module_t  ngx_http_live_stat_module_ctx = {
    NULL,                                    /* preconfiguration */
    ngx_http_live_stat_postconfiguration,    /* postconfiguration */
    ...
    ngx_http_live_stat_create_loc_conf,      /* create location configuration */
    ngx_http_live_stat_merge_loc_conf,       /* merge location configuration */
};


ngx_module_t  ngx_http_live_stat_module = {
    NGX_MODULE_V1,
    &ngx_http_live_stat_module_ctx,          /* module context */
    ngx_http_live_stat_commands,             /* module directives */
...
static void *
ngx_http_live_stat_escape(ngx_http_request_t *r, void *data, size_t len)
{
    u_char *p, *np;
    void   *new_data;
    size_t  n;
...
static void
ngx_http_live_stat_output(ngx_http_request_t *r, ngx_chain_t ***lll,
                     void *data, size_t len, ngx_uint_t escape)
{
    ...
    if (escape) {
        data = ngx_http_live_stat_escape(r, data, len);
        if (data == NULL) {
            return;
        }
    }
...
/* plain data */
#define NGX_HTTP_LIVE_STAT(data, len)    ngx_http_live_stat_output(r, lll, data, len, 0)

/* escaped data */
#define NGX_HTTP_LIVE_STAT_E(data, len)  ngx_http_live_stat_output(r, lll, data, len, 1)

/* literal */
#define NGX_HTTP_LIVE_STAT_L(s)          NGX_HTTP_LIVE_STAT((s), sizeof(s) - 1)

/* ngx_str_t */
#define NGX_HTTP_LIVE_STAT_S(s)          NGX_HTTP_LIVE_STAT((s)->data, (s)->len)

/* escaped ngx_str_t */
#define NGX_HTTP_LIVE_STAT_ES(s)         NGX_HTTP_LIVE_STAT_E((s)->data, (s)->len)

/* C string */
#define NGX_HTTP_LIVE_STAT_CS(s)         NGX_HTTP_LIVE_STAT((s), ngx_strlen(s))

/* escaped C string */
#define NGX_HTTP_LIVE_STAT_ECS(s)        NGX_HTTP_LIVE_STAT_E((s), ngx_strlen(s))


#define NGX_HTTP_LIVE_STAT_BW            0x01
#define NGX_HTTP_LIVE_STAT_BYTES         0x02
#define NGX_HTTP_LIVE_STAT_BW_BYTES      0x03
...
static void
ngx_http_live_stat_get_pool_size(ngx_pool_t *pool, ngx_uint_t *nlarge,
                            ngx_uint_t *size)
{
    ngx_pool_large_t       *l;
    ngx_pool_t             *p, *n;
...
static void
ngx_http_live_stat_dump_pool(ngx_http_request_t *r, ngx_chain_t ***lll,
                        ngx_pool_t *pool)
{
    ...
    size = 0;
    nlarge = 0;
    ngx_http_live_stat_get_pool_size(pool, &nlarge, &size);
    NGX_HTTP_LIVE_STAT_L("<pool><nlarge>");
    NGX_HTTP_LIVE_STAT(buf, ngx_snprintf(buf, sizeof(buf), "%ui", nlarge) - buf);
    NGX_HTTP_LIVE_STAT_L("</nlarge><size>");
    NGX_HTTP_LIVE_STAT(buf, ngx_snprintf(buf, sizeof(buf), "%ui", size) - buf);
    NGX_HTTP_LIVE_STAT_L("</size></pool>\r\n");
}
#endif


static void
ngx_http_live_stat_client(ngx_http_request_t *r, ngx_chain_t ***lll)
{
    u_char  buf[NGX_INT_T_LEN];
...
#endif
    NGX_HTTP_LIVE_STAT_L("<id>");
    NGX_HTTP_LIVE_STAT(buf, ngx_snprintf(buf, sizeof(buf), "%ui",
                                    (ngx_uint_t) r->connection->number) - buf);
    NGX_HTTP_LIVE_STAT_L("</id>");
    
    NGX_HTTP_LIVE_STAT_L("<address>");
    NGX_HTTP_LIVE_STAT_ES(&r->connection->addr_text);
    NGX_HTTP_LIVE_STAT_L("</address>");
    
//    NGX_HTTP_LIVE_STAT_L("<time>");
//    NGX_HTTP_LIVE_STAT(buf, ngx_snprintf(buf, sizeof(buf), "%i",
//                                    (ngx_int_t) (ngx_current_msec - r->epoch)) - buf);
//    NGX_HTTP_LIVE_STAT_L("</time>");
...
static void
ngx_http_live_stat_live(ngx_http_request_t *r, ngx_chain_t ***lll,
                   ngx_http_live_loc_conf_t *llcf)
{
    ...
    slcf = ngx_http_get_module_loc_conf(r, ngx_http_live_stat_module);
    
    NGX_HTTP_LIVE_STAT_L("<live>\r\n");
    
    total_nclients = 0;
    for (n = 0; n < llcf->nbuckets; ++n) {
        for (stream = llcf->streams[n]; stream; stream = stream->next) {
            NGX_HTTP_LIVE_STAT_L("<stream>\r\n");
            
            NGX_HTTP_LIVE_STAT_L("<name>");
            NGX_HTTP_LIVE_STAT_ECS(stream->name);
            NGX_HTTP_LIVE_STAT_L("</name>\r\n");
            ...
            for (ctx = stream->ctx; ctx; ctx = ctx->next, ++nclients) {
                rr = ctx->r;
                if (slcf->stat & NGX_HTTP_LIVE_STAT_CLIENTS) {
                    NGX_HTTP_LIVE_STAT_L("<client>");
                    
                    ngx_http_live_stat_client(rr, lll);
                    ...
                    if (ctx->publishing) {
                        NGX_HTTP_LIVE_STAT_L("<publishing/>");
                    }
            ...
            NGX_HTTP_LIVE_STAT_L("<nclients>");
            NGX_HTTP_LIVE_STAT(buf, ngx_snprintf(buf, sizeof(buf),
                                            "%ui", nclients) - buf);
            NGX_HTTP_LIVE_STAT_L("</nclients>\r\n");
...
static void
ngx_http_stat_location(ngx_http_request_t *r, ngx_chain_t ***lll,
                       ngx_http_core_loc_conf_t *clcf)
{
    ngx_http_live_loc_conf_t       *llcf;
    
    
    llcf =  clcf->loc_conf[ngx_http_live_module.ctx_index];
    if (llcf == NULL || !llcf->live) {
        return;
    }
    
    NGX_HTTP_LIVE_STAT_L("<location>\r\n");
    NGX_HTTP_LIVE_STAT_L("<name>");
    NGX_HTTP_LIVE_STAT_ES(&clcf->name);
    NGX_HTTP_LIVE_STAT_L("</name>\r\n");
    
    ngx_http_live_stat_live(r, lll, llcf);
...
static void
ngx_http_stat_static_location(ngx_http_request_t *r, ngx_chain_t ***lll,
                              ngx_http_location_tree_node_t *node)
{
    ...
    if (clcf) {
        ngx_http_stat_location(r, lll, clcf);
    }
    
    ngx_http_stat_static_location(r, lll, node->left);
    ngx_http_stat_static_location(r, lll, node->right);
    ngx_http_stat_static_location(r, lll, node->tree);
}
                     ...
                     ngx_http_core_srv_conf_t *cscf)
{
    ...
    // only support static locations right now
    if (clcf && clcf->static_locations) {
        ngx_http_stat_static_location(r, lll, clcf->static_locations);
    }
...
ngx_http_live_stat_handler(ngx_http_request_t *r)
{
    ...
    NGX_HTTP_LIVE_STAT_L("<?xml version=\"1.0\" encoding=\"utf-8\" ?>\r\n");
    if (slcf->stylesheet.len) {
        NGX_HTTP_LIVE_STAT_L("<?xml-stylesheet type=\"text/xsl\" href=\"");
        NGX_HTTP_LIVE_STAT_ES(&slcf->stylesheet);
...
#ifdef NGX_COMPILER
    NGX_HTTP_LIVE_STAT_L("<compiler>" NGX_COMPILER "</compiler>\r\n");
#endif
    NGX_HTTP_LIVE_STAT_L("<built>" __DATE__ " " __TIME__ "</built>\r\n");
    
    NGX_HTTP_LIVE_STAT_L("<pid>");
    NGX_HTTP_LIVE_STAT(nbuf, ngx_snprintf(nbuf, sizeof(nbuf),
                                     "%ui", (ngx_uint_t) ngx_getpid()) - nbuf);
    NGX_HTTP_LIVE_STAT_L("</pid>\r\n");
    
    NGX_HTTP_LIVE_STAT_L("<uptime>");
    NGX_HTTP_LIVE_STAT(tbuf, ngx_snprintf(tbuf, sizeof(tbuf),
                                     "%T", ngx_cached_time->sec - start_time) - tbuf);
    NGX_HTTP_LIVE_STAT_L("</uptime>\r\n");
    
//    NGX_HTTP_LIVE_STAT_L("<naccepted>");
//    NGX_HTTP_LIVE_STAT(nbuf, ngx_snprintf(nbuf, sizeof(nbuf),
...
static void *
ngx_http_live_stat_create_loc_conf(ngx_conf_t *cf)
{
    ngx_http_live_stat_loc_conf_t       *conf;
    
    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_live_stat_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static char *
ngx_http_live_stat_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_live_stat_loc_conf_t       *prev = parent;
    ngx_http_live_stat_loc_conf_t       *conf = child;
...
static char *
ngx_http_live_stat(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_core_loc_conf_t  *clcf;
    
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
    clcf->handler = ngx_http_live_stat_handler;
    
    return ngx_conf_set_bitmask_slot(cf, cmd, conf);
}

static ngx_int_t
ngx_http_live_stat_postconfiguration(ngx_conf_t *cf)
{
